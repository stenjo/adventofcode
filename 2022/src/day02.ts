...
export class RPSGame {
    ...
    Strategy(gameRound: string): number {
        ...
        // Score = choice + win/draw/loss
        const scoreMap:{round:string, score:number}[] = [
            {round:'A X', score: 0+3}, // loss(0) + scisso(3)
            {round:'A Y', score: 3+1}, // draw(3) + rock(1)
            {round:'A Z', score: 6+2}, // win(6) + paper(2)
            {round:'B X', score: 0+1}, // loss(0) + rock(1)
            {round:'B Y', score: 3+2}, // draw(3) + paper(2)
            {round:'B Z', score: 6+3}, // win(6) + scissor(3)
            {round:'C X', score: 0+2}, // loss(0) + paper(2)
            {round:'C Y', score: 3+3}, // draw(3) + scissor(3)
            {round:'C Z', score: 6+1}, // win(6) + rock(1)
        ];

        let result = scoreMap.find(({round}) => round === gameRound)?.score
        if (result != undefined) {
            return result;
        }
        return 0;
