...
class Machine:
    def __init__(self, input):
        ...
        for (name, module) in [(name,module) for name,module in self.modules.items() if module.type == "&"]:
            module.setSenders(self.getSenders(name))
        
        self.modules["rx"] = Module(None, [])
    ...
    def minButtonPresses(self):
        presses = 0
        while self.modules["rx"].signalReceived in [None] and presses < 10000000:
            presses += 1
            self.signal(False)
