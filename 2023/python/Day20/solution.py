...
INPUT_PATH = Path(SCRIPT_PATH, "../../data/input20.txt")
    ...
    for target in targets:
        if target in modules.keys():  # Only process if there is a module for the target
            if target == "broadcaster":  # Maintain strength for broadcaster
                ...
                modules[target]["type"] == "%"
            ):  # For flip-flop, if turning on send high pulse, and if turning off send low pulse
                if strength == "L":
...
def part1(parsed_data):
    ...
    for _ in range(1000):
        ...
        ]  # Start with the pulse from the button
        while queue:
            q = queue.pop(0)
            match q[1]:
                case "L":
...
def part2(parsed_data):
    ...
    for (
        ...
        module_dict.keys()
    ):  # Find the modules that output to the final conjunction module and set those to False
    ...
    while not all(
        final_inputs.values()
    ):  # While all final inputs are still false, loop
        count += 1
        ...
        while queue:
            ...
            for item in process_pulse(module_dict, *q):
                queue.append(item[0])
                module_dict = item[1]
            for key in module_dict[final_con][
                "status"
            ]:  # Check the conjunction modules that lead into final_con and if they have a high pulse, record how many presses it took
                if (
                    not final_inputs[key]
                    and module_dict[final_con]["status"][key] == "H"
                ):
                    print(
                        f"Module {key} took {count} number of button presses before it produced a high pulse"
                    )
...
if __name__ == "__main__":
    ...
    print(
        f"PART2 - The number of button presses to start the final machine is {answer2}"
    )
